...
class ResolutionWindow(QWidget):
    ...
    def set_table(self):
        ...
        for row, roi in enumerate(
            sorted(rois, key=lambda r: r.fit.fwhm if r.fit else -1)
        ):
            table.insertRow(row)
    ...
    def calculate(self):
        ...
        for i in range(self.data_table.rowCount()):
            if (
                not self.data_table.cellWidget(i, 0)
                .layout()
                .itemAt(0)
                .widget()
                .isChecked()
                or self.data_table.item(i, 4).text() == "-"
            ):
                continue
        ...
        self.current_resolution_points = np.array(
            [fwhm / e for fwhm, e in zip(fwhms, centroids)]
        )
    ...
    def plot_data(self, mode: int):
        ...
        if mode == 0:
            self.res_plot.getPlotItem().plot(
                self.current_energy_points,
                self.current_fwhm_points,
                pen=None,
                symbol="o",
            )
            ...
            self.res_plot.getPlotItem().plot(
                self.current_energy_points,
                self.current_resolution_points * 100,
                pen=None,
                symbol="o",
            )
