# Forked from https://github.com/physkael/digiBase/blob/main/digibase.py
# Copyright (c) 2024 Kael Hanson (kael.hanson@gmail.com)
# Modified for inclusion in LuRaCs
...
class digiBase:
    ...
    def __init__(self, firmware_dir_path: str | Path, serialNumber=None):
        ...
        if self.isRH:
            ...
            firmware_file_path = firmware_dir_path / "digiBaseRH.rbf"
        else:
            r = self.send_command(b'\x06')
            needs_init = (r[0] == 0)
            firmware_file_path = firmware_dir_path / "digiBase.rbf"

        if needs_init:
            with firmware_file_path.open('rb') as f:
                fw = f.read()
            if self.isRH:
...
if __name__ == "__main__":
    ...
    base = digiBase("/home/eewa/Documents/git/MySpect/.appdata/third_party_drivers_library")
    ...
    try:
        while True:
            print(f"Live time {base.livetime}, Real time {base.realtime}")
            spec = base.spectrum
            line.set_data(range(len(spec)), spec)
