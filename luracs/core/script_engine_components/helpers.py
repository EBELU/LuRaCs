        ...
        while i < len(args):
            ...
            if arg_key.startswith("-"):
                if arg_key not in self.signatures:
                    raise ArgumentError(
                        f"{arg_key} is not a valid optional argument for {self.command}, valid arguments are {self.signatures.keys()}"
                    )
...
class TableFormatter:
    def __init__(
        self, col_titles: list, col_types: list | None = None, title: str | None = None
    ):
    ...
    def get_table(self):
        # Header
        header = " | ".join(
            f"{title:<{self.col_widths[i]}}" for i, title in enumerate(self.col_titles)
        )

        # Separator
        separator = "-+-".join(
            "-" * self.col_widths[i] for i in range(len(self.col_titles))
        )
        ...
        for row in self.rows:
            formatted_row = " | ".join(
                f"{str(cell):<{self.col_widths[i]}}" for i, cell in enumerate(row)
            )
...
def plot_spectrum(args):
    ...
    if spectrum_name == "all" or kwargs["-A"]:
        ...
        for name, spectrum in SpectrumManager.get_spectra_dict().items():
            fg = (
                spectrum.get_foreground(kwargs["-log"], kwargs["-cps"])
                if not kwargs["-bkgSub"]
                else spectrum.get_bkg_sub(kwargs["-log"])
            )
        ...
        if spectrum is None:
            try:
                ...
                raise ArgumentError(
                    f"Given index is out of range for spectrum list containing {len(SpectrumManager.get_spectra_dict())} item(s)"
                )
