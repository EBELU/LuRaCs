...
class NuclideLibrary(QObject):
    ...
    def match_energy_to_nuclide(self, energy: float, match_only_shown = True, window: float = 50) -> Emission:
        ...
        if not match_only_shown or len(self.selected_nuclides) == 0:
            ...
            for nuclide_name in self.selected_nuclides:
                emissions.extend([e for e in self.get_nuclide(nuclide_name).emissions if abs(e.energy_keV - energy) < window])
    ...
    def match_roi_to_nuclide(self, roi_tag: str, energy_search_window: float = 50):
        ...
        matches = [self.match_energy_to_nuclide(r.fit.mu, window=energy_search_window) for r in rois.values() if r.fit is not None]
