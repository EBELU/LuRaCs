...
class InstrumentLibrary(QObject):
    ...
    def __init__(self, instrument_class: UniqueInstrument | GenericInstrument, parent = None, ):
    ...
    def set_instrument_from_parser(self, key: str, parser: xml_parser):
        if key not in self.instrument_registry:
            instrument = parser.get_instrument()
            
            assert isinstance(instrument, self._instrument_class), f"Instrument is class {type(instrument)}"
    ...
    def rename_instrument(self, old_instrument_key: str, new_instrument: UniqueInstrument | GenericInstrument):
