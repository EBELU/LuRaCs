...
class _FileIndex(QObject):
    def __init__(self, parent):
        ...
        # Build the indexes
        self.spectrum_index = Indexer(
            self,
            Settings.Paths.spectrum_library / "*.xml",
            {"meta_only": True},
            save_spectrum_to_library,
            xml_parser,
        )
        self.spectrogram_index = Indexer(
            self, Settings.Paths.spectrogram_library / "*.db", {}, None, db_parser
        )
        self.roi_index = Indexer(
            self,
            Settings.Paths.roi_library / "*.xml",
            {"meta_only": True},
            save_rois_to_internal,
            xml_parser,
        )
        self.unique_instrument_index = Indexer(
            self,
            Settings.Paths.unique_instrument_library / "*.xml",
            {"meta_only": True},
            save_instrument_to_library,
            xml_parser,
        )
        self.generic_instrument_index = Indexer(
            self,
            Settings.Paths.generic_instrument_library / "*.xml",
            {"meta_only": True},
            save_instrument_to_library,
            xml_parser,
        )
        ...
        self.unique_instrument_index.sigItemAdded.connect(
            SpectrumManager.UniqueInstrumentLibrary.set_instrument_from_parser
        )
        self.generic_instrument_index.sigItemAdded.connect(
            SpectrumManager.GenericInstrumentLibrary.set_instrument_from_parser
        )

        # Deleting
        SpectrumManager.UniqueInstrumentLibrary.sigRemoveInstrument.connect(
            self.unique_instrument_index.delete_file
        )
        SpectrumManager.GenericInstrumentLibrary.sigRemoveInstrument.connect(
            self.generic_instrument_index.delete_file
        )

        # Updating existing
        SpectrumManager.UniqueInstrumentLibrary.sigInstrumentUpdated.connect(
            self.unique_instrument_index.update_file
        )
        SpectrumManager.GenericInstrumentLibrary.sigInstrumentUpdated.connect(
            self.generic_instrument_index.update_file
        )

        # Saving a new
        SpectrumManager.UniqueInstrumentLibrary.sigNewInstrumentAdded.connect(
            self.unique_instrument_index.save_file
        )
        SpectrumManager.GenericInstrumentLibrary.sigNewInstrumentAdded.connect(
            self.generic_instrument_index.save_file
        )

        # Rename
        SpectrumManager.UniqueInstrumentLibrary.sigInstrumentRenamed.connect(
            self.unique_instrument_index.rename_file
        )
        SpectrumManager.GenericInstrumentLibrary.sigInstrumentRenamed.connect(
            self.generic_instrument_index.rename_file
        )
    ...
    def run_index_all(self):
        ...
        for indexer in [
            ...
            self.roi_index,
            self.unique_instrument_index,
            self.generic_instrument_index,
        ]:
            indexer.run_index()
            gui_logger.debug(
                f"{indexer.__class__}: {len(indexer.index_registry)} files found in {indexer.path}"
            )
...
class Indexer(QObject):
    ...
    def rename_file(self, old_name: str, new_name: str):
        ...
        if new_name in self.index_registry:
            ...
            os.remove(new_name)
            gui_logger.debug(
                f"{self.__class__} File {new_name} removed to rename {old_name}"
            )
    ...
    def update_file(self, key: str, new_item):
        ...
        gui_logger.debug(
            f"{self.__class__} Update completed! Key: {key}, item {new_item}"
        )
        self.sigIndexUpdated.emit()
...
class _Importer(QObject):
    ...
    # --- Generic Importers ---
    def import_files(
        self, filter=None, dialog_parent=None
...
class _Exporter(QObject):
    ...
    def export_roi_dialog(self):
        ...
        for spectrum in SpectrumManager.spectrum_registry:
            rois.extend(
                SpectrumManager.ROIManager.get_data_from_spectrum(spectrum).values()
            )
