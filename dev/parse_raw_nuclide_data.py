    ...
    with open(file) as f:
        ...
        for line in f.readlines():
            ...
            match line_parts[0]:
                ...
                case "Half-life (s)":
                    json_dict["Half-life (s)"] = [float(line_parts[1]), float(line_parts[2])]

                case "Specific activity (Bq/g)":
                    json_dict["Specific activity (Bq/g)"] = [float(line_parts[1]), float(line_parts[2])]
                ...
                case _:
                    if "--------" in line_parts[0]:
                        ...
                        if float(line_parts[0]) < 20 or float(line_parts[2]) < 1:
                            continue
                        emissions.append({"Energy (keV)": float(line_parts[0]),
                                          "Energy error (keV)": float(line_parts[1]) if line_parts[1] else -1,
                                          "I (%)": float(line_parts[2]),
                                          "I error (%)": float(line_parts[3]) if line_parts[3] else -1,
